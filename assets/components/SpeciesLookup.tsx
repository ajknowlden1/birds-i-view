...
export default function SpeciesLookup(props: any) {
  ...
  useEffect(() => {
    getBirdBySpeciesCode(speciesCode).then((res: any) => {
  ...
  if (!birdPicked) {
    ...
    return (
      <View style={styles.container}>
        ...
        <TouchableOpacity style={styles.button} onPress={speciesNav}>
          <Text styles={styles.text}>View Bird Profile</Text>
        </TouchableOpacity>
        <Text style={styles.text}>
          Recent sightings nationwide: {sightings.length}
        </Text>
        <TouchableOpacity style={styles.button} onPress={mapNav}>
          <Text styles={styles.text}>View on Map</Text>
        </TouchableOpacity>
      </View>
    );
  }
